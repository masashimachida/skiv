...
class StartCommand extends Command_1.default {
    ...
    async execute() {
        ...
        (0, utils_1.sendKeys)(0, ["cd", seWorkspace]);
        (0, utils_1.sendKeys)(0, ["claude", "--model", model]);
     ...
     */
    createWorkspace(rootDir, name, role) {
        const workspaceDir = path_1.default.join(rootDir, 'workspaces', name);
        const roleDir = path_1.default.join(rootDir, 'roles', role);
        if (fs_1.default.existsSync(workspaceDir))
            return workspaceDir;
        (0, child_process_1.execSync)(`cp -Rp ${roleDir} ${workspaceDir}`);
        return workspaceDir;
    }
    /**
     ...
     */
    setupTmuxGrid(paneCount) {
        for (let i = 1; i < paneCount; i++) {
            (0, child_process_1.execSync)('tmux split-window -f -v');
        }
        (0, child_process_1.execSync)('tmux select-layout main-horizontal');
        (0, child_process_1.execSync)('tmux set-window-option main-pane-height 40%');
        (0, child_process_1.execSync)('tmux select-pane -t 0');
    }
}
